...
class SnailfishNumber {
  public:
    // Does not support the public interface, used only for allocation
...
{
  std::vector<SnailfishNumber> nums;
  ...
  while (!std::cin.eof()) {
    ...
    nums.push_back(SnailfishNumber(buf));
  }

  int maxMag = 0;
  SnailfishNumber maxMagNum;
  for (auto i = 0; i < nums.size(); ++i) {
    for (auto j = 0; j < nums.size(); ++j) {
      if (i == j)
        continue;
      SnailfishNumber num = nums[i];
      num += nums[j];
      if (num.GetMagnitude() > maxMag) {
        maxMag = num.GetMagnitude();
        maxMagNum = num;
      }
    }
  }

  std::cout << "Answer = " << maxMag << "\n";
  std::cout << "Enjoy the visual tree of answer:\n";
  maxMagNum.DrawTree();
