...
int main(int argc, char *argv[])
{
  ...
  std::vector<long long int> current(N_TYPES * N_TYPES, 0);
  std::vector<long long int> tmp(N_TYPES * N_TYPES, 0);
  ...
  std::vector<long long int> count(N_TYPES, 0);
  for (int i = 0; i < N_TYPES; ++i) {
    int offset = i * N_TYPES;
    for (int j = 0; j < N_TYPES; ++j) {
      int idx = j + offset;
  ...
  long long int ans = count.back() - count.front();
  printf("Answer = %lld\n", ans);
